...
"
    ...
    <form wire:submit="updateProfile">
        ...
        <div class="relative mb-32 md:mb-24 -mx-4 md:-mx-8 -mt-4 md:-mt-8">
            ...
            <div class="absolute -bottom-24 md:-bottom-16 left-1/2 md:left-8 -translate-x-1/2 md:translate-x-0 flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 w-full md:w-auto px-4 md:px-8">
                <div class="relative shrink-0 group">
                    ...
                    @else
                    <img id="profile-preview" src="{{ Auth::user()->foto ? asset('storage/'.Auth::user()->foto) : 'https://images.pexels.com/photos/2876486/pexels-photo-2876486.png?auto=compress&cs=tinysrgb&dpr=1&w=500' }}"
                        alt="Profile"
                        class="w-32 h-32 rounded-full border-[8px] border-[#37517e] md:border-[#37517e] bg-gray-600 object-cover shadow-lg transition-filter duration-300">
                    @endif
                    <label id="pfp-label" for="pfp-upload"
                        :class="isEditMode ? 'group-hover:opacity-100 cursor-pointer pointer-events-auto' : 'pointer-events-none'"
                ...
                <div class="pb-2 text-center md:text-left">
                    <h2 id="display-name-header" class="text-2xl font-bold text-[#37517e]">{{ Auth::user()->namalengkap ?? 'User' }}</h2>
                    <p class="text-[#37517e]/70 text-sm capitalize">{{ Auth::user()->usertype ?? 'Guest' }}</p>
                </div>
            </div>
            <div class="absolute -bottom-36 md:-bottom-12 left-1/2 md:left-auto md:right-8 -translate-x-1/2 md:translate-x-0 flex flex-row items-center gap-2">
                <button
                    id="edit-master-btn"
                    type="button"
        ...
        <div class="bg-[#2d4268] rounded-2xl p-4 md:p-6 mt-20 md:mt-6 shadow-inner border border-[#415a8a]">
            <div class="divide-y divide-white/10 space-y-2">
                ...
                <div class="py-1 first:pt-0">
                    <p class="text-[10px] md:text-[11px] uppercase font-black text-blue-200/50 tracking-[0.1em] mb-2">Nama Lengkap</p>
                    <input type="text" wire:model="namalengkap"
                        :readonly="!isEditMode"
                        :class="isEditMode ? 'bg-white/10 border-solid border-white/20 focus:ring-2 focus:ring-blue-400' : 'border-none pointer-events-none'"
                ...
                <div id="password-section" x-show="isEditMode" x-transition class="pt-4">
                    ...
                    <div class="py-1" x-data="{ show: false }">
                        <p class="text-[10px] md:text-[11px] uppercase font-black text-blue-200/50 tracking-[0.1em] mb-2">Password Lama</p>
                        ...
                        <div class="relative flex gap-2">
                            <div class="relative w-full">
                                ...
                                <button type="button" @click="show = !show" wire:ignore class="absolute right-3 top-1/2 -translate-y-1/2 text-white/70 hover:text-white transition-colors outline-none">
                            ...
                            @if(!$passwordVerified)
                            <button type="button"
                                wire:click.prevent="verifyOldPassword"
                                class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-bold text-xs transition-colors shadow-lg">
                                CEK
                            </button>
                            @else
                            <div class="bg-emerald-500/20 border border-emerald-500 text-emerald-400 px-4 py-2 rounded-lg flex items-center justify-center">
                    ...
                    <div class="mt-4 space-y-2 border-t border-white/5 pt-2" x-transition.opacity>
                        ...
                        <div class="py-1" x-data="{ show: false }">
                            <p class="text-[10px] md:text-[11px] uppercase font-black text-blue-200/50 tracking-[0.1em] mb-2">Password Baru</p>
                            <div class="relative">
                                <input :type="show ? 'text' : 'password'"
                                    wire:model="password" x-model="newPass"
                        ...
                        <div class="py-1" x-data="{ show: false }">
                            <p class="text-[10px] md:text-[11px] uppercase font-black text-blue-200/50 tracking-[0.1em] mb-2">Ulangi Password Baru</p>
                            <div class="relative">
                                <input :type="show ? 'text' : 'password'"
                                    wire:model="password_confirmation" x-model="confirmPass"
                                    placeholder="Ulangi password baru"
                                    :class="passwordsMatch === false ? '!border-red-500 focus:ring-red-500' : (passwordsMatch === true ? '!border-emerald-500 focus:ring-emerald-500' : 'border-white/20 focus:ring-emerald-400')"
                            ...
                            <p x-show="passwordsMatch === true" class="text-[10px] mt-1 text-emerald-400 font-bold">Password baru cocok!</p>
                            <p x-show="passwordsMatch === false" class="text-[10px] mt-1 text-red-400 font-bold">Password baru tidak sama!</p>
                        </div>
                    </div>
                    @endif
