...
document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("siswaData", (initialStudents, routeStore, csrfToken, usertype, currentSiswaId) => ({
        ...
        get filteredStudents() {
            ...
            if (this.sortByPoin !== '') {
                result = result.sort((a, b) => {
                    ...
                    return this.sortByPoin === 'desc' ? poinB - poinA : poinA - poinB;
                });
            } else {
                result = result.sort((a, b) => {
                    let valA = a[this.sortCol].toString().toLowerCase();
        ...
        get drawerTitle() {
            ...
            if (this.drawerMode === "tambahPelanggaran") return "Catat Pelanggaran";
        ...
        async handleFileUpload(event, pelanggaranId) {
            ...
            try {
                ...
                if (file.size > 500 * 1024) {
                    ...
                    console.log(`Foto dikompres dari ${(file.size / 1024).toFixed(0)}KB ke ${sizeKB}KB`);
        ...
        removePhoto(pelanggaranId) {
            ...
            const fileInput = document.getElementById('file-' + pelanggaranId);
            if (fileInput) {
                fileInput.value = '';
            }
        ...
        async deletePelanggaran(id_pelanggaran) {
            ...
            try {
                const response = await fetch(`/pelanggaran/${id_pelanggaran}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Content-Type': 'application/json'
        ...
        async savePelanggaranBatch() {
            ...
            try {
                ...
                for (let pelanggaranId in this.selectedPelanggaran) {
                    const pelanggaran = this.selectedPelanggaran[pelanggaranId];
                    ...
                    if (this.photoFiles[pelanggaranId]) {
                        formData.append('bukti_foto', this.photoFiles[pelanggaranId]);
                    }
